...
export default async function NewServicePage({ params: { locale }, searchParams }: { params: { locale: string }, searchParams?: { error?: string } }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-3xl font-bold gold-gradient">{isEn ? 'Add New Service' : 'إضافة خدمة جديدة'}</h1>
      </div>
      ...
      <div className="glass p-8 rounded-xl border border-primary/20">
        <form action={createServiceAction.bind(null, locale)} className="space-y-6">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <textarea name="excerpt_en" required rows={2} className="w-full bg-background border border-primary/20 rounded-md px-4 py-2 focus:outline-none focus:border-primary transition-colors"></textarea>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1 text-primary">{isEn ? 'Excerpt (Arabic)' : 'ملخص (عربية)'}</label>
                <textarea name="excerpt_ar" required rows={2} className="w-full bg-background border border-primary/20 rounded-md px-4 py-2 focus:outline-none focus:border-primary transition-colors"></textarea>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <textarea name="description_en" required rows={4} className="w-full bg-background border border-primary/20 rounded-md px-4 py-2 focus:outline-none focus:border-primary transition-colors"></textarea>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1 text-primary">{isEn ? 'Description (Arabic)' : 'الوصف (عربية)'}</label>
                <textarea name="description_ar" required rows={4} className="w-full bg-background border border-primary/20 rounded-md px-4 py-2 focus:outline-none focus:border-primary transition-colors"></textarea>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input name="expected_result_en" type="text" required className="w-full bg-background border border-primary/20 rounded-md px-4 py-2 focus:outline-none focus:border-primary transition-colors" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1 text-primary">{isEn ? 'Expected Result (Arabic)' : 'النتيجة المتوقعة (عربية)'}</label>
                <input name="expected_result_ar" type="text" required className="w-full bg-background border border-primary/20 rounded-md px-4 py-2 focus:outline-none focus:border-primary transition-colors" />
              </div>
            </div>
          ...
          <div className="pt-4 border-t border-primary/10">
            <SubmitButton 
              idleText={isEn ? 'Create Service' : 'إنشاء خدمة'} 
